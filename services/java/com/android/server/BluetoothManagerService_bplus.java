...
class BluetoothManagerService_bplus extends IBluetoothManager.Stub {
    ...
    private static final int MESSAGE_USER_SWITCHED_POSTPONE = 301;
    ...
    BluetoothManagerService_bplus(Context context) {
            ...
            switch (msg.what) {
                ...
                case MESSAGE_USER_SWITCHED:
                {
                    ...
                    if ( (mEnable || isRadioEnabled()) && mBluetooth != null) {
                        ...
                        Message switchMsg = mHandler.obtainMessage(MESSAGE_USER_SWITCHED_POSTPONE);
                        mHandler.sendMessageDelayed(switchMsg, USER_SWITCHED_TIME_MS);
                ...
                case MESSAGE_USER_SWITCHED_POSTPONE:
                {
                    if (DBG) {
                        Log.d(TAG, "MESSAGE_USER_SWITCHED_POSTPONE");
                    }
                    mHandler.removeMessages(MESSAGE_USER_SWITCHED_POSTPONE);
                    if (mState == BluetoothAdapter.STATE_RADIO_OFF) {
                        synchronized (mConnection) {
                            if (mBluetooth != null) {
                                //Unregister callback object
                                try {
                        ...
                        mHandler.sendMessageDelayed(switchMsg, USER_SWITCHED_TIME_MS);
                        if (DBG) {
                            Log.d(TAG, "delay MESSAGE_USER_SWITCHED_POSTPONE");
                        }
                    }
                    break;
                }

         ...
         public void enter() {
             ...
             if (mAuthenticationFailuresCount >= MAX_RETRIES_ON_AUTHENTICATION_FAILURE) {
                 ...
                 Log.d(TAG, "Association getting rejected, disabling network " +
                         stateChangeResult.networkId);
                 //gwl 
		 Log.d(TAG, "Disabled network's ssid : " + stateChangeResult.wifiSsid);
                 boolean inscanresult = false;
		 List<ScanResult> results = mWifiStateMachine.syncGetScanResultsList();
                 if (results != null) {
                     for (ScanResult result : results) { 
                         if(result.SSID.equals(stateChangeResult.wifiSsid)) {
                             inscanresult = true;
                             handleNetworkConnectionFailure(stateChangeResult.networkId,
                     ...
                     if (!inscanresult)
                         Log.d(TAG, "Disabled network is not in scanresults, cancel disable this network.");
                 }
                 mAssociationRejectCount = 0;
            }
         }

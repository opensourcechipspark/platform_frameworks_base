...
public class WifiNative {
    ...
    private native boolean connectToSupplicantNative(String iface);

    private native void closeSupplicantConnectionNative(String iface);

    /**
     * Wait for the supplicant to send an event, returning the event string.
     * @return the event string sent by the supplicant.
     */
    private native String waitForEventNative(String iface);

    private native boolean doBooleanCommandNative(String iface, String command);

    private native int doIntCommandNative(String iface, String command);

    private native String doStringCommandNative(String iface, String command);
    ...
    public boolean connectToSupplicant() {
        ...
        return connectToSupplicantNative(mInterfaceName);
    }

    public void closeSupplicantConnection() {
        if (VDBG) localLog(mInterfacePrefix + "closeSupplicantConnection");
        closeSupplicantConnectionNative(mInterfaceName);
    }
    ...
    private boolean doBooleanCommand(String command) {
        ...
        synchronized (mLock) {
            ...
            boolean result = doBooleanCommandNative(mInterfaceName, command);
    ...
    private int doIntCommand(String command) {
        ...
        synchronized (mLock) {
            int cmdId = getNewCmdIdLocked();
            if (VDBG) localLog(cmdId + "->" + mInterfacePrefix + command);
            int result = doIntCommandNative(mInterfaceName, command);
            if (VDBG) localLog(cmdId + "<-" + result);
            if (DBG) Log.d(mTAG, "   returned " + result);
            return result;
        }
    ...
    private String doStringCommand(String command) {
        ...
        synchronized (mLock) {
            int cmdId = getNewCmdIdLocked();
            if (VDBG) localLog(cmdId + "->" + mInterfacePrefix + command);
            String result = doStringCommandNative(mInterfaceName, command);
            if (VDBG) localLog(cmdId + "<-" + result);
            if (DBG) Log.d(mTAG, "   returned " + result);
            return result;
        }
    ...
    private String doStringCommandWithoutLogging(String command) {
        if (DBG) Log.d(mTAG, "doString: " + command);
        synchronized (mLock) {
            return doStringCommandNative(mInterfaceName, command);
        }
    }
    ...
    public boolean scan(int type) {
        if (type == SCAN_WITHOUT_CONNECTION_SETUP) {
			//doBooleanCommand("DRIVER SCAN-PASSIVE");
    ...
    public String scanResults(int sid) {
        return doStringCommandWithoutLogging("BSS RANGE=ALL MASK=0x1986");
